...
{
    public class GoodsOutStorageMap : EntityTypeConfiguration<GoodsOutStorage>
    {
        public GoodsOutStorageMap()
        {
            Property(p => p.SourceNumber).HasColumnName("F_SourceNumber");
            // 字典
            Property(p => p.GoodsStatus).HasColumnName("F_GoodsState").HasColumnType("smallint");
            Property(p => p.OutStorageType).HasColumnName("F_OutType").HasColumnType("smallint");
            // 外键
