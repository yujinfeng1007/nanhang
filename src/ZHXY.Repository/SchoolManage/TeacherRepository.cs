...
{
    public class TeacherRepository : Data.Repository<Teacher>, ITeacherRepository
    {
        ...
        public new void Delete(string keyValue)
        {
            using (var db = new Data.UnitWork().BeginTrans())
            {
                ...
                for (var i = 0; i < F_Id.Length; i++)
                {
                    ...
                    if (model != null)
                    {
                        db.Delete<SysUser>(t => t.F_Id == model.F_User_ID);
                        db.Delete(model);
                ...
                foreach (var data in datas)
                    if (data.F_Id.IsEmpty())
                    {
                        ...
                        var user = new SysUser
                        {
                            ...
                            F_RoleId = "teacher",
                            F_DepartmentId = data.F_Divis_ID,
                            //F_MobilePhone = data.,
                    ...
                    else
                    {
                        ...
                        var user = db.FindEntity<SysUser>(t => t.F_Id == data.F_User_ID);
                        user.F_Gender = Convert.ToBoolean(Convert.ToInt32(data.F_Gender));
                        user.F_DepartmentId = data.F_Divis_ID;
                ...
                if (data.F_Id.IsEmpty())
                    try
                    {
                        using (var db = new Data.UnitWork().BeginTrans())
                        {
                            ...
                            {
                                db.Delete<SysUserRole>(t => t.F_User == data.F_User_ID);
                    ...
                    try
                    {
                        using (var db = new Data.UnitWork().BeginTrans())
                        {
                            ...
                            var user = db.FindEntity<SysUser>(t => t.F_Id == teachentity.F_User_ID);
        ...
        public void AddDatasImport(SysUser userentity, SysUserLogin userlogentity, Teacher entity)
        {
            using (var db = new Data.UnitWork().BeginTrans())
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(entity.F_RoleId))
                    {
                        ...
                        for (var i = 0; i < F_RoleId.Length - k; i++)
                        {
                            ...
                            us.F_Role = GetRoleListByCache().FirstOrDefault(q =>
                                GetPropertyValue(q.Value, "fullname").Equals(F_RoleId[i].ToString())).Key;
                            db.Delete<SysUserRole>(t => t.F_User == entity.F_User_ID);
                            db.Insert(us);
                        }
                    }
