...
namespace ZHXY.Application
{
    ...
    public static class TreeHelper
    {
        ...
        {
            app.Read<Organize>(p => p.F_ParentId.Equals(rootId)).Select(p => p.F_Id).ToList().ForEach(e=>
            {
                result.Add(e);
        ...
        public static List<TreeView> GetSubOrg(this AppService app,  string nodeId=null ,int nodeLevel=0)
        {
            ...
            return  app.Read<Organize>(p => p.F_ParentId.Equals(nodeId)).Select(p =>
                new TreeView
                {
                    Id = p.F_Id,
                    Parent = p.F_ParentId,
