...
//    {
//        public SysUserAppService(IZhxyRepository r) : base(r)
//        {
//        }
...
//        {
//            var query = Read<SysUser>(t => t.F_DepartmentId.Equals("parent") && t.F_Account != "admin");
...
//        {
//            return Read<SysUser>().ToListAsync().Result;
//        }

//        public SysUser GetByAccount(string account)
//        {
//            return Read<SysUser>(t => t.F_Account.Equals(account)).FirstOrDefaultAsync().Result;
//        }

//        public List<SysUser> GetByOrg(string orgId, bool recursive = true)
//        {
//            var query = Read<SysUser>(p => p.F_Account != "admin");
//            if (string.IsNullOrEmpty(orgId)) return query.ToList();
//            var orgs = recursive ? OrgApp.GetListByParentId(orgId)?.Select(p => p.F_Id).ToList() : Read<SysOrganize>(p => p.F_ParentId.Equals(orgId)).Select(p => p.F_Id).ToListAsync().Result;
//            orgs.Add(orgId);
//            return query.Where(p => orgs.Contains(p.F_DepartmentId)).ToListAsync().Result;
//        }


//        public SysUser Get(string id) => Read<SysUser>(p => p.F_Id.Equals(id)).FirstOrDefaultAsync().Result;

//        public string GetRoleById(string id) => Read<SysUser>(p => p.F_Id.Equals(id)).FirstOrDefault()?.F_RoleId;

//        public int UpdateByStudentNum(string studentNum, DateTime updateTime, string file)
//        {
//            var user = Query<SysUser>(p => p.F_Account.Equals(studentNum)).FirstOrDefaultAsync().Result;
...
//            var removeList = Query<SysUser>(p => idList.Contains(p.F_Id)).ToListAsync().Result;
...
//            }
//            DelAndSave<SysUser>(removeList);
//        }

//        public void Submit(SysUser userEntity, SysUserLogin userLogOnEntity, string keyValue)
//        {
...
//                userRoles.AddRange(from r in roles where !r.IsEmpty() select new SysUserRole { F_Id = Guid.NewGuid().ToString("N").ToUpper(), F_Role = r, F_User = userEntity.F_Id });
//            }
//            UserRepository.SubmitForm(userEntity, userLogOnEntity, keyValue, userRoles);
//        }

//        public void SubmitSetUp(SysUser userEntity)
//        {
//            userEntity.Modify(OperatorProvider.Current.UserId);
//            UserRepository.Update(userEntity);
//        }

//        public void ParentSubmit(SysUser userEntity, SysUserLogin userLogOnEntity, string id)
//        {
//            if (Read<SysUser>(t => t.F_Account == userEntity.F_Account).Any()) throw new Exception("该账号已存在");
...
//        public void Update(SysUser userEntity)
//        {
//            UserRepository.Update(userEntity);
//        }
...
//        {
//            var user = Read<SysUser>(p => p.F_Account.Equals(username)).FirstOrDefaultAsync().Result;
//            if (user == null) throw new Exception("账户不存在，请重新输入");
//            if (user.F_EnabledMark != true) throw new Exception("账户被系统锁定,请联系管理员");
//            var userLogin = Get<SysUserLogin>(user.F_Id);
...
//        public SysUser SsoLogin(string username)
//        {
//            var user = Read<SysUser>().FirstOrDefaultAsync(t => t.F_Account == username).Result;
//            if (user == null) throw new Exception("账户不存在，请重新输入");
//            if (user.F_EnabledMark != true) throw new Exception("账户被系统锁定,请联系管理员");
//            //对接航天恒嘉门户登录时，下段注释代码会出错，entity和数据库匹配有问题
//            //var userLogin = Get<SysUserLogin>(user.F_Id);
//            //userLogin.F_PreviousVisitTime = userLogin.F_LastVisitTime.HasValue ? userLogin.F_LastVisitTime : null;
...
//        {
//            var userLogin = Read<SysUserLogin>(p => p.F_Id.Equals(userid)).Select(p => new { SecretKey = p.F_UserSecretkey, Password = p.F_UserPassword }).FirstOrDefaultAsync().Result;
...
//        {
//            var user = Read<SysUser>(p => p.F_Id.Equals(Id)).FirstOrDefault();
