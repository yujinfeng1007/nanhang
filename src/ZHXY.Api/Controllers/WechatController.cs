...
namespace ZHXY.Api.Controllers
{
    ...
    public class WechatController : BaseApiController
    {
        ...
        {
            var student = new StudentAppService().GetBykeyValue(F_StudentNo);
            var pwd = "000000";
            if (!student.F_CredNum.IsEmpty() && student.F_CredNum.Length >= 6)
            {
                pwd = student.F_CredNum.Substring(student.F_CredNum.Length - 6, 6);
