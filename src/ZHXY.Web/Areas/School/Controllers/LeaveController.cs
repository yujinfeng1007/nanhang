...
namespace ZHXY.Web.School.Controllers
{
    ...
    public class LeaveController : ZhxyWebControllerBase
    {
        ...
        {
            var (list, records, total) = App.GetLeaveHistory(input);
            return PagingResult(list, records, total);
        });
        ...
        {
            input.CurrentUserId = OperatorProvider.Current.UserId;
            var (list, recordCount, pageCount) = App.GetApprovalList(input);
        ...
        {
            input.CurrentUserId = OperatorProvider.Current.UserId;
            var (list, recordCount, pageCount) = App.GetCanceList(input);
            return Resultaat.PagingRst(list, recordCount, pageCount);
        }
