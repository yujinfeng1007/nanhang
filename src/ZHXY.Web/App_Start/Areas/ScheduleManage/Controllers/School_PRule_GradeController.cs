...
{
    public class School_PRule_GradeController : ControllerBase
    {
        ...
        private School_PRule_Grade_App ruleApp = new School_PRule_Grade_App();
        ...
        public ActionResult GradeRule()
        {
            return View();
        }

        public ActionResult GeGradeRule(string F_Divis, string F_Grade, string F_Year, string F_Class, string F_Semester)
        {
            var data = ruleApp.GetList(F_Divis, F_Grade, F_Year, F_Semester);
            return Content(data.ToJson());
        }

        public ActionResult SetGradeRule(string F_Divis, string F_Grade, string F_Year, string F_Class, string F_Semester, List<SubmitModel> data)
        {
            SetRule(1, null, null, F_Divis, F_Grade, F_Year, F_Class, F_Semester, data);
            return Success("操作成功。");
        }
        ...
        {
            ruleApp.DeleteForm(F_Year, F_Semester, F_Grade);
            if (datas == null)
                return;
            foreach (var data in datas)
            {
                Schedule_PRule_Grade_Entity model = new Schedule_PRule_Grade_Entity();
                model.Create();
                //model.F_Type = F_Type;
                model.F_Year = F_Year;
