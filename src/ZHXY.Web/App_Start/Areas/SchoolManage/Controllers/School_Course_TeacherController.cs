/*******************************************************************************
 * Author: mario
 * Description: School_Course_Teacher  Controller类
********************************************************************************/
...
{
    //科目l老师关联表
    public class School_Course_TeacherController : ControllerBase
    {
        private School_Course_Teacher_App app = new School_Course_Teacher_App();
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridSelect(string F_CourseID)
        {
            var data = app.GetSelect(F_CourseID);
            return Content(data.ToJson());
        }
        ...
        public ActionResult SubmitForm(string F_Teacher, string keyValue, string F_CourseID)
        {
            app.SubmitForm(F_Teacher, keyValue, F_CourseID);
            return Success("操作成功。");
        }
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Course_Teacher", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Course_Teacher列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Course_Teacher列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            List<CourseTeacher> list = ExcelToList<CourseTeacher>(Server.MapPath(filePath), rules);
        ...
        public ActionResult GetTeacherByCourse(string F_Course)
        {
            var data = app.GetList(t => t.F_CourseID == F_Course).Select(t => t.School_Teachers_Entity).ToList();
            return Content(data.ToJson());
        }
    }
}