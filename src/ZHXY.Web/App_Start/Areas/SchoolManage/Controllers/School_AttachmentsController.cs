/*******************************************************************************
 * Author: mario
 * Description: School_Attachments  Controller类
********************************************************************************/
...
{
    //资源库
    public class School_AttachmentsController : ControllerBase
    {
        private School_Attachments_App app = new School_Attachments_App();
        ...
        [HttpGet]
        [HandlerAuthorize]
        public virtual ActionResult MyForm()
        {
            string keyValue = Request["keyValue"];
            if (!Ext.IsEmpty(keyValue))
            {
                SchoolAttachment attach = app.GetForm(keyValue);
                OperatorModel user = OperatorProvider.Provider.GetCurrent();
                //自己的资源可以修改
                if (attach.F_Uploader == user.UserId)
                {
                    return View();
                }
                else
                {
                    return Content("无修改权限");
                }
            }
            return View();
        }
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, string keyword, string F_Semester, string F_Type)
        {
            var data = new
            {
                rows = app.GetList(pagination, keyword, F_Semester, F_Type),
        ...
        [HttpGet]
        //[HandlerAjaxOnly]
        public ActionResult GetMyGridJson(Pagination pagination, string keyword, string F_Semester, string F_Type)
        {
            var data = new
            {
                rows = app.GetMyList(pagination, keyword, F_Semester, F_Type),
                total = pagination.Total,
                page = pagination.Page,
                records = pagination.Records
            };
        ...
        public ActionResult SubmitForm(SchoolAttachment entity, string keyValue)
        {
        ...
        [HandlerAjaxOnly]
        [ValidateAntiForgeryToken]
        public ActionResult MySubmitForm(SchoolAttachment entity, string keyValue)
        {
            app.SubmitForm(entity, keyValue);
            return Success("操作成功。");
        }
        ...
        [HandlerAjaxOnly]
        [ValidateAntiForgeryToken]
        public ActionResult MyDeleteForm(string keyValue)
        {
            ...
            for (int i = 0; i < F_Id.Length - 1; i++)
            {
                if (!Ext.IsEmpty(keyValue))
                {
                    SchoolAttachment attach = app.GetForm(F_Id[i]);
                    //自己的资源可以修改
                    if (attach.F_Uploader == user.UserId)
                    {
                    }
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Attachments", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Attachments列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Attachments列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_Name", new string[] { "附件名称", "" });
            //rules.Add("F_Path", new string[] { "附件路径", "" });
            //rules.Add("F_Appear_Type", new string[] { "公开方式(个人、班级、校级)", "" });
            //rules.Add("F_Tag", new string[] { "内容标签（课件、私有、通知、其他）", "" });
            //rules.Add("F_Type", new string[] { "附件类型（doc/docx/ppt/pptx/pdf/txt/href/avi/mp4/rar/png/jpg/jepg）", "" });
            //rules.Add("F_File_Size", new string[] { "文件大小", "" });
            //rules.Add("F_Semester", new string[] { "学期", "" });
            //rules.Add("F_School", new string[] { "学校ID", "" });
            //rules.Add("F_Uploader", new string[] { "上传人", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头
            List<SchoolAttachment> list = ExcelToList<SchoolAttachment>(Server.MapPath(filePath), rules);
