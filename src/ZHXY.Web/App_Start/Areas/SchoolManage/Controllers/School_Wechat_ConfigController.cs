/*******************************************************************************
 * Author: mario
 * Description: School_Wechat_Config  Controller类
********************************************************************************/
...
{
    //微信配置
    public class School_Wechat_ConfigController : ControllerBase
    {
        private WechatConfigApp app = new WechatConfigApp();
        ...
        public ActionResult SubmitForm(WechatConfig entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Wechat_Config", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.GetDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Wechat_Config列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Wechat_Config列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_School_Name", new string[] { "学校名称", "" });
            //rules.Add("F_Wechat_AppId", new string[] { "appid", "" });
            //rules.Add("F_Wechat_", new string[] { "学期名称", "" });
            //rules.Add("F_Wechat_NotifyUrl", new string[] { "学期别名", "" });
            //rules.Add("F_EncodingAESKey", new string[] { "EncodingAESKey", "" });
            //rules.Add("F_Wechat_Pay_Appkey", new string[] { "微信支付appKwy", "" });
            //rules.Add("F_Wechat_MerchantNo", new string[] { "商户号", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<WechatConfig> list = ExcelToList<WechatConfig>(Server.MapPath(filePath), rules);
