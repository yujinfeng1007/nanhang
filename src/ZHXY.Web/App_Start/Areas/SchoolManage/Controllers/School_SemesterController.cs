/*******************************************************************************
 * Author: mario
 * Description: School_Semester  Controller类
********************************************************************************/
...
{
    //学期表
    public class School_SemesterController : ControllerBase
    {
        private School_Semester_App app = new School_Semester_App();
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridSelectByF_Year(string F_Year)
        {
            var data = app.GetListByF_Year(F_Year);
            return Content(data.ToJson());
        }
        ...
        public ActionResult SubmitForm(Semester entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Semester", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Semester列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Semester列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_Year", new string[] { "学年度", "" });
            //rules.Add("F_Name", new string[] { "学期名称", "" });
            //rules.Add("F_Nickname", new string[] { "学期别名", "" });
            //rules.Add("F_Start_Time", new string[] { "开始时间", "" });
            //rules.Add("F_End_Time", new string[] { "结束时间", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<Semester> list = ExcelToList<Semester>(Server.MapPath(filePath), rules);
