/*******************************************************************************
 * Author: mario
 * Description: School_Info  Controller类
********************************************************************************/
...
{
    //学校信息
    public class School_InfoController : ControllerBase
    {
        private School_Info_App app = new School_Info_App();
        ...
        [ValidateInput(false)]
        public ActionResult SubmitForm(SchoolInfo entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Info", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Info列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Info列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_Name", new string[] { "学校名称", "" });
            //rules.Add("F_Address", new string[] { "地址", "" });
            //rules.Add("F_Logo", new string[] { "logo", "" });
            //rules.Add("F_School_History", new string[] { "学校历史", "" });
            //rules.Add("F_School_Memo", new string[] { "学校概况", "" });
            //rules.Add("F_Semester", new string[] { "当前学期", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<SchoolInfo> list = ExcelToList<SchoolInfo>(Server.MapPath(filePath), rules);
