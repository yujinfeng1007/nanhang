/*******************************************************************************
 * Author: mario
 * Description: School_Class_Duty  Controller类
********************************************************************************/
...
{
    //班级值日表
    public class School_Class_DutyController : ControllerBase
    {
        private School_Class_Duty_App app = new School_Class_Duty_App();
        ...
        public ActionResult SubmitForm(ClassDuty entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Class_Duty", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Class_Duty列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Class_Duty列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_Date", new string[] { "值日日期", "" });
            //rules.Add("F_Student", new string[] { "学生ID", "" });
            //rules.Add("F_Kp", new string[] { "值日考评", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头
            List<ClassDuty> list = ExcelToList<ClassDuty>(Server.MapPath(filePath), rules);
