/*******************************************************************************
 * Author: mario
 * Description: School_ChangeBill_Log  Controller类
********************************************************************************/
...
{
    //报名过程中修改学部造成的收费记录，收费变更记录表
    public class School_ChangeBill_LogController : ControllerBase
    {
        private School_ChangeBill_Log_App app = new School_ChangeBill_Log_App();

        public ActionResult JFForm()
        {
            return View();
        }

        public ActionResult XJForm()
        {
            return View();
        }

        public ActionResult TKForm()
        {
            return View();
        }
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, string keyword, string billId)
        {
            var data = new
            {
                rows = app.GetList(pagination, keyword, billId),
        ...
        public ActionResult SubmitForm(ChangeBillLog entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_ChangeBill_Log", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_ChangeBill_Log列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_ChangeBill_Log列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_No", new string[] { "变更编号", "" });
            //rules.Add("F_Invit_No", new string[] { "申请编号", "" });
            //rules.Add("F_Students_ID", new string[] { "学生", "" });
            //rules.Add("F_Name", new string[] { "姓名", "" });
            //rules.Add("F_StudentNum", new string[] { "学号", "" });
            //rules.Add("F_Divis_ID", new string[] { "学部", "" });
            //rules.Add("F_Grade_ID", new string[] { "年级", "" });
            //rules.Add("F_Class_ID", new string[] { "班级", "" });
            //rules.Add("F_Bill_Change_Type", new string[] { "变更类型", "" });
            //rules.Add("F_ Proof", new string[] { "变更凭证", "" });
            //rules.Add("F_ Proof_Memo", new string[] { "变更原因", "" });
            //rules.Add("F_Description", new string[] { "变更备注", "" });
            //rules.Add("F_Fee", new string[] { "变更金额", "" });
            //rules.Add("F_Charge_Id", new string[] { "缴费账单", "" });
            ...
            List<ChangeBillLog> list = ExcelToList<ChangeBillLog>(Server.MapPath(filePath), "School_ChangeBill_Log");
