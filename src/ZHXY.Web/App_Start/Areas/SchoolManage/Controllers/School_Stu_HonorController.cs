/*******************************************************************************
 * Author: mario
 * Description: School_Stu_Honor  Controller类
********************************************************************************/
...
{
    //学生荣誉表
    public class School_Stu_HonorController : ControllerBase
    {
        ...
        private School_Stu_Honor_App app = new School_Stu_Honor_App();
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJsonByTea(Pagination pagination, string keyword, string F_DepartmentId, string F_CreatorTime_Start, string F_CreatorTime_Stop)
        {
            var data = new
            {
                rows = app.GetListByTea(pagination, keyword, F_DepartmentId, F_CreatorTime_Start, F_CreatorTime_Stop),
                total = pagination.Total,
                page = pagination.Page,
                records = pagination.Records
            };
        ...
        public ActionResult SubmitForm(StudentHonor entity, string keyValue)
        {
        ...
        [HandlerAjaxOnly]
        [ValidateAntiForgeryToken]
        public ActionResult AuditorForm(StudentHonor entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Stu_Honor", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Stu_Honor列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Stu_Honor列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_CreatorTime", new string[] { "创建时间", "" });
            //rules.Add("F_CreatorUserId", new string[] { "申请填写人", "" });
            ...
            //rules.Add("F_Auditor_Time", new string[] { "审核时间", "" });
            //rules.Add("F_Divis_ID", new string[] { "学生学部", "" });
            //rules.Add("F_Grade_ID", new string[] { "学生年级", "" });
            //rules.Add("F_Class_ID", new string[] { "学生班级", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<StudentHonor> list = ExcelToList<StudentHonor>(Server.MapPath(filePath), rules);
