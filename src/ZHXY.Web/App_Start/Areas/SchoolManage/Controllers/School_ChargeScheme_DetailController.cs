/*******************************************************************************
 * Author: mario
 * Description: School_ChargeScheme_Detail  Controller类
********************************************************************************/
...
{
    //收费标准明细
    public class School_ChargeScheme_DetailController : ControllerBase
    {
        private School_ChargeScheme_Detail_App app = new School_ChargeScheme_Detail_App();

        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, string keyword, string F_ChargeScheme_ID)
        {
            var data = new
            {
                rows = app.GetList(pagination, F_ChargeScheme_ID),
        ...
        public ActionResult SubmitForm(ChargeSchemeDetail entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            pagination.Page = 1;
            List<ChargeSchemeDetail> list = app.GetList(pagination, keyword);
            ...
            //所有字段代码
            //rules.Add("F_Id", new string[] { "明细ID", "" });
            //rules.Add("F_ChargeScheme_ID", new string[] { "收费标准ID", "" });
            //rules.Add("F_Fees_Name", new string[] { "收费项目名称", "" });
            //rules.Add("F_Fees", new string[] { "收费金额", "" });
            ...
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_ChargeScheme_Detail列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_ChargeScheme_Detail列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<ChargeSchemeDetail> list = ExcelToList<ChargeSchemeDetail>(Server.MapPath(filePath), rules);
