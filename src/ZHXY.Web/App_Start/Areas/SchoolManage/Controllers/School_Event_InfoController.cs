/*******************************************************************************
 * Author: mario
 * Description: School_Event_Info  Controller类
********************************************************************************/
...
{
    //活动信息表
    public class School_Event_InfoController : ControllerBase
    {
        private School_Event_Info_App app = new School_Event_Info_App();

        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, string keyword, string F_CreatorTime_Start, string F_CreatorTime_Stop, string F_Class, string F_EventType)
        {
            var data = new
            {
                rows = app.GetList(pagination, keyword, F_CreatorTime_Start, F_CreatorTime_Stop, F_Class, F_EventType),
        ...
        [ValidateInput(false)]
        public ActionResult SubmitForm(Activity entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Event_Info", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Event_Info列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Event_Info列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_EventType", new string[] { "活动类别（班级、学校）", "" });
            //rules.Add("F_Title", new string[] { "活动标题", "" });
            //rules.Add("F_Content", new string[] { "活动内容", "" });
            //rules.Add("F_Cover", new string[] { "活动简图", "" });
            //rules.Add("F_Imgs", new string[] { "活动图片集合", "" });
            //rules.Add("F_School", new string[] { "班级ID", "" });
            //rules.Add("F_Class", new string[] { "学校ID", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<Activity> list = ExcelToList<Activity>(Server.MapPath(filePath), rules);
