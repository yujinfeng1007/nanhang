/*******************************************************************************
 * Author: mario
 * Description: School_Presence_Info  Controller类
********************************************************************************/
...
{
    //风采信息表
    public class School_Presence_InfoController : ControllerBase
    {
        ...
        private School_Presence_Info_App app = new School_Presence_Info_App();

        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, string keyword, string F_Pattern, string F_EnabledMark, string F_CreatorTime_Start, string F_CreatorTime_Stop, string F_Class, string F_PresenceType)
        {
            var data = new
            {
                rows = app.GetList(pagination, keyword, F_Pattern, F_EnabledMark, F_CreatorTime_Start, F_CreatorTime_Stop, F_Class, F_PresenceType),
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetIndexGridJson(Pagination pagination, string keyword, string F_Pattern, string F_EnabledMark, string F_CreatorTime_Start, string F_CreatorTime_Stop, string F_Class, string F_PresenceType)
        {
            var data = new
            {
                rows = app.GetIndexList(pagination, keyword, F_Pattern, F_EnabledMark, F_CreatorTime_Start, F_CreatorTime_Stop, F_Class, F_PresenceType),
                total = pagination.Total,
                page = pagination.Page,
                records = pagination.Records
            };
        ...
        [ValidateInput(false)]
        public ActionResult SubmitForm(Presence entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("School_Presence_Info", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "School_Presence_Info列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "School_Presence_Info列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            //rules.Add("F_PresenceType", new string[] { "风采类别（师生风采、班级荣誉、班级风采、作品展示、校园风采）", "" });
            //rules.Add("F_Content", new string[] { "风采内容", "" });
            //rules.Add("F_Files", new string[] { "风采文件链接集合", "" });
            //rules.Add("F_ Pattern", new string[] { "文件格式（视频、图片）", "" });
            //rules.Add("F_Class", new string[] { "班级", "" });
            //rules.Add("F_School", new string[] { "学校ID", "" });

            //////////////////处理数据(机构 岗位 等字典替换，过滤不要的字段，修改表头)
            List<Presence> list = ExcelToList<Presence>(Server.MapPath(filePath), rules);
