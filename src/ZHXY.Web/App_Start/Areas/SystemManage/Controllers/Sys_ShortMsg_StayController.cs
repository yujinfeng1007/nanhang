...
{
    //Sys_ShortMsg_Stay
    public class Sys_ShortMsg_StayController : ControllerBase
    {
        private Sys_ShortMsg_Stay_App app = new Sys_ShortMsg_Stay_App();
        ...
        public ActionResult SubmitForm(SysShortMsgStay entity, string keyValue)
        {
        ...
        public FileResult export(string keyword)
        {
            ...
            string exportSql = CreateExportSql("Sys_ShortMsg_Stay", parms);
            //string exportSql = "";
            //Console.WriteLine("exportSql==>" + exportSql);
            DataTable dt = app.getDataTable(exportSql, dbParameter);
            ///////////////////写流
            MemoryStream ms = new NPOIExcel().ToExcelStream(dt, "Sys_ShortMsg_Stay列表");
            ms.Seek(0, SeekOrigin.Begin);
            string filename = "Sys_ShortMsg_Stay列表" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        ...
        public ActionResult import(string filePath)
        {
            ...
            List<SysShortMsgStay> list = ExcelToList<SysShortMsgStay>(Server.MapPath(filePath), rules);
